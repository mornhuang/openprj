    ...
    *accurately* compared to other version strings. Take, for example, the
        ...
        if version:
            if isinstance(version, Version):
                ...
                if match:
                    ...
                    if match.group(4):
                        self._prerelease = tuple(map(_value, match.group(4).split('.')))

                    if match.group(5):
                        self._build = tuple(map(_value, match.group(5).split('.')))
                else:
    ...
    def __lt__(self, other):
        ...
        if self._prerelease and not other._prerelease:
            ...
            return False
        elif not self._prerelease and not other._prerelease:
            return False
